...
from keras.layers import CuDNNLSTM, TimeDistributed, RepeatVector, Dense
import numpy as np
import wandb
...
model = Sequential()
model.add(CuDNNLSTM(config.hidden_size, input_shape=(maxlen, len(chars))))
model.add(RepeatVector(config.digits + 1))
model.add(CuDNNLSTM(config.hidden_size, return_sequences=True))
